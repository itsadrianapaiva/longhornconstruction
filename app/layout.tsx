...
export default async function RootLayout({
  ...
  // ---- FIX: safely type the nav slice of the dictionary
  // We avoid accessing dict.nav.hero directly because the dictionary's type
  // may not declare `nav`. Create a local structural type for the nav keys.
  type NavKey =
    | "hero"
    | "about"
    | "sismo"
  ...
  // Approx sticky header height for scroll offset
      ...
      <body className="min-h-screen bg-white text-zinc-900 antialiased">
        <header className="sticky top-0 z-40 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
            ...
            <Suspense
              fallback={<div className="h-8 w-14 rounded-full bg-zinc-100" />}
            >
